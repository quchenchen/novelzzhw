...
async def test_minglou_spy_scenario():
    ...
    # 获取数据库引擎
    engine = await get_engine('test_user')
    ...
    async with AsyncSessionLocal() as db:
        # ============ 1. 创建测试项目 ============
        test_name = f"伪装者-明楼测试项目-{datetime.now().strftime('%Y%m%d%H%M%S')}"
        project = Project(
        ...
        if memory:
            ...
            print(f"✅ 验证通过：测试chapter不存在，记忆记录跳过（符合预期）")
...
async def test_same_character_different_orgs():
    ...
    async with AsyncSessionLocal() as db:
        # 创建测试项目
        project = Project(
            user_id="test_user_for_identity_system",
            title=f"多重身份组织测试-{datetime.now().strftime('%Y%m%d%H%M%S')}",
            genre="武侠"
        )
        db.add(project)
        await db.commit()
