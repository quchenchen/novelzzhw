...
<naming_guide priority="P1">
【角色命名指南】
根据项目类型和主题，生成符合特色的名字，避免使用陈词滥调的网文风格名字。

**武侠/仙侠主题：**
- 姓名+名：萧峰、郭靖、杨过、令狐冲风格（传统姓氏+富有寓意的名字）
- 复姓更佳：慕容、皇甫、欧阳、司马、上官、宇文、独孤
- 名字寓意：使用诗词典故或武学术语，如"破天""傲天""惊鸿""慕云"
- 避免：林晚、陈默、苏墨等网文常见名字
- 女性特色：灵珊、语嫣、盈盈、婉儿、若兰

**现代都市主题：**
- 现实姓名：张伟、李娜、王芳、刘洋、陈静等常见但不过时的名字
- 特色名字：文艺气质如"雨桐""梦琪""安然""子涵""思源"
- 职业关联：律师可用"正""诚"，医生可用"仁心""济世"
- 避免：过于文艺或矫情的名字

**古风/架空主题：**
- 雅致古风：墨染、清歌、扶摇、归荑、望舒、未央
- 皇室贵胄：轩辕、长孙、独孤、宇文（复姓更显尊贵）
- 文人雅士：子卿、怀瑾、守拙、忘机
...
**奇幻/魔法主题：**
- 西幻风格：艾尔文、索拉、泰兰德拉、凯尔萨斯
- 东方奇幻：云梦、星落、月痕、风语、雪见

**历史题材：**
- 符合时代特征：汉代多用单名，唐宋后多用字辈
- 避免现代感强的名字
- 考虑社会地位：平民简朴、贵族华美
...
✅ 便于记忆但不过度特殊
✅ 避免同时出现多个发音相似的名字
✅ 禁用"林晚""陈默""苏墨"等过度使用的网文名
</naming_guide>

<organization_naming_guide priority="P1">
【组织命名指南】
根据项目类型和主题，生成符合特色的组织名称。

**武侠/仙侠：**
- 使用"门派、帮、盟、宗、教、阁"等后缀
- 示例：逍遥派、长风盟、天机阁、峨眉派、神教、青云宗

**现代都市：**
- 使用"公司、集团、工作室、机构"等
- 示例：远洋集团、晨光工作室、金融科技、传媒机构

**古风/架空：**
- 使用"阁、轩、楼、殿、府、山庄"等
- 示例：听雨阁、青云轩、望月楼、凌云殿、紫阳府

**科幻未来：**
- 使用"公司、组织、联盟、集团、局"等
- 示例：星际联盟、量子科技、联邦局、合成集团

**奇幻/魔法：**
- 使用"教会、议会、公会、学院、教团"等
- 示例：光明教会、元素议会、冒险者公会、魔法学院

**历史题材：**
- 符合时代特征的称谓
- 示例：XX府、XX帮、XX会、XX门、XX商号
</organization_naming_guide>
...
<identity_guide priority="P1">
【隐藏身份分析】
根据角色背景和故事需求，判断是否需要为角色设置隐藏身份。
...
**身份类型说明：**
- real（真身）：角色的真实身份，最本质的存在
- public（公开）：大家熟知的公开身份
- secret（隐藏）：只有极少数人知道的秘密身份
- disguise（伪装）：临时使用的伪装身份

**如果角色需要隐藏身份，在JSON中添加identities字段：**
{{
  "identities": [
    {{
      "name": "真实姓名/主身份",
    ...
    {{
      ...
      "status": "active",
      "appearance": "伪装后的外貌",
      "background": "伪装身份的设定背景"
    }}
  ]
}}

**注意事项：**
- 如果不需要隐藏身份，则不需要添加identities字段（系统会自动创建默认主身份）
- 每个角色至少需要一个主身份（is_primary=true）
- 隐藏身份的organization_memberships应该使用该身份加入组织
</identity_guide>
...
{{
  ...
  "identities": [
    {{
      "name": "身份名称",
      "identity_type": "real/public/secret/disguise",
      "is_primary": true,
      "status": "active",
      "appearance": "外貌描述",
      "personality": "性格表现",
...
【身份信息说明】
- 如果角色有多个身份（如卧底、双重身份等），请在identities中列出
- identity_type可选值：real（真身）/public（公开）/secret（隐藏）/disguise（伪装）
- 每个角色至少需要一个主身份（is_primary=true）
- 如果只有一个身份，可以不填写identities字段，系统会自动创建默认身份
...
- 使用"门派、帮、盟、宗、教、阁"等后缀
- 示例：逍遥派、长风盟、天机阁、峨眉派、神教、青云宗
- 名称风格：富有武学或道家色彩，如"紫霄""凌云""归元""玄天"

**现代都市：**
- 使用"公司、集团、工作室、机构"等
- 示例：远洋集团、晨光工作室、金融科技、传媒机构
- 名称风格：体现行业特色或愿景，避免过于空泛
...
每个身份暴露需要：
- character_name：角色名称
- exposed_identity_name：被暴露的身份名称
- exposure_type：暴露类型（secret_revealed隐藏暴露/disguise_broken伪装识破/revealed_to特定人知晓）
- exposure_context：暴露的具体场景描述
- witnesses：目击者角色名列表
- impact_on_organization：对组织关系的影响描述
...
{{
  ...
  "identity_exposures": [
    {{
      "character_name": "李四",
      "exposed_identity_name": "暗夜使者",
      "exposure_type": "secret_revealed",
      "exposure_context": "在战斗中面具被击落，露出真实面容",
      "witnesses": ["张三", "王五"],
      "impact_on_organization": "天剑门可能追杀"
    }}
  ],
  "plot_points": [
    {{
...
❌ 职业设计与世界观脱节
    ...
    # 番茄小说平台专用写作指南
    FANQIE_WRITING_GUIDE = """<fanqie_guide priority="P1">
【番茄小说平台写作指南】
